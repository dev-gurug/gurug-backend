...
router.post("/update", [auth], async (req, res) => {
  const group = await GPost.findByIdAndUpdate(
    req.body.id,
    {
      title: req.body.title,
      description: req.body.description,
      groupPostImage: req.body.groupPostImage,
    },
...
router.delete("/:id", [auth], async (req, res) => {
  try {
    const gpost = await GPost.findByIdAndDelete(req.params.id);
    if (!gpost) return res.status(404).send("GPost does not exist...");
    res.status(200).send();
  } catch (error) {
    res.status(400).send(error.message);
  }
...
router.put("/post/incrementComments/:id", [auth], async (req, res) => {
  ...
  try {
    post = await GPost.findByIdAndUpdate(
      post._id,
